...
async def async_download(site, cdpath, usr, pwd):
    ...
    tasks = [asyncio.ensure_future(fetch_async(sem, i, filename)) for i, filename in enumerate(filenames)]  #Create series of coroutines tasks (download tasks) to execute
...
for orgAccount, siteURL in ftpSites.items():
    ...
    if 'cryosat' in siteURL:
        satName = 'cryosat'
        cdPath = pathlib.Path("SIR_GDR", "2017", "01")
    else:
        raise ValueError('unknown satellite')

    #Make directory if not already exist
    pathlib.Path.mkdir(pathlib.Path(dataDir / satName / cdPath), parents=True, exist_ok=True)
